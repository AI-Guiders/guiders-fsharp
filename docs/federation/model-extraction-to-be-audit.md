...
**Summary (ship-59):** verified **25** / **25** audit rows · partial **0** · gap **0** · **CLOSURE: CLOSED**
...
| P1-09 | Notations.Bracket — Kind: canon; delete BracketAnchorSpan, AnchorWire, SniperScope | verified | Kind: + `BracketResolveBoundary`; `LegacyWireSpan` + `LegacyNavWireIngest` deleted; `RelationWireBoundary.TryParseDocScan` doc ingest only; nav Kind:Nav only | Phase1 deletes; `CorrespondenceWireTests`, `DocReverseAnchorResolverTests`, Phase2/10 gates |
